...
def edit(sno):
  ...
  if request.method == 'POST':
      ...
      if sno == '0':
          post = Posts(title=box_title, slug=slug, content=content, tagline=tline, img_file=image.filename,date=datetime.now(pytz.timezone('Asia/Kolkata')).strftime("%Y-%m-%d %H:%M:%S"),owner_id=current_user.get_id())
          db.session.add(post)
          db.session.commit()
      else:
          ...
          post.date=datetime.now(pytz.timezone('Asia/Kolkata')).strftime("%Y-%m-%d %H:%M:%S")
